terraform {
 required_providers {
  iactools={
     source = "registry.terraform.io/lffsc123/iactools"
     version = "1.2.37"
   } 
 }
 }
...
resource "iactools_RealServiceList" "cs2" {
 poollist={
  name="string__*"
  monitor="string" // 健康监测
  rs_list="string"
...
resource "iactools_AddrPoolList" "cs" {
addrpoollist={
    name="string__*"
    ip_start="string__*"
  	ip_end="string__*"
...
resource "iactools_VirtualService" "cs" {
    virtualservice={
      name ="string__*"
      mode ="string__*"
      ip ="string__*"
...
resource "iactools-TargetNat" "dpcs" {
  targetnat={
    name="string__*"
    ip_start="string__*"
    ip_end="string__*"
...
resource "iactools-SourceNat" "dpcs" {
  sourcenat={
    name="string__*"
    ip_start="string__*"
    ip_end="string__*"
...
// 会话保持
resource "iactools-SessionKeep" "dpcs" {
  sessionkeep={
    // 1）必须配置。
    // 2）会话保持策略名称。
...
// 健康监测
resource "iactools-AdxSlbMonitor" "dpcs" {
  monitorinfo={
    // 1）必须配置
    // 2）新建健康监测的名称
    // 3）最大支持63个字符，不可以包涵
